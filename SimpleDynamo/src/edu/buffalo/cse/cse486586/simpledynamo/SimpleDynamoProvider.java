...
public class SimpleDynamoProvider extends ContentProvider {
	...
	Uri myUri = Uri.parse("content://" + SimpleDynamoProvider.AUTHORITY + "/MessageT");
	...
	public static final String AUTHORITY1 = "edu.buffalo.cse.cse486586.simpledynamo.provider1";
	...
	private static final String sqlCreateDB = "CREATE TABLE " + tableName + "("
			+ KeyValueBase.MSG_ID + " INTEGER PRIMARY KEY AUTOINCREMENT,"
			+ KeyValueBase.provider_key + " LONGTEXT," + KeyValueBase.provider_value
			+ " LONGTEXT );";
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		key = insertV.getAsString(KeyValueBase.provider_key);
		value = insertV.getAsString(KeyValueBase.provider_value);
				...
				if (!retHash.contentEquals(myHash)) {
					...
					if (rID > 0) {
						...
						getContext().getContentResolver().notifyChange(
								KeyValueBase.CONTENT_URI, null);
						AccessGlobalData.isDBEmpty = false;
						Log.d("ClientThread",
								"Local Put Operation Successfull for key "
										+ key);
						isKeyValid[Integer.parseInt(key)] = true;

						if (!AccessGlobalData.isDataReplicated) {
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		count = db.update(tableName, values,
				KeyValueBase.MSG_ID + " = " + selection, null);
		getContext().getContentResolver().notifyChange(uri, null);
		return count;
	}
...
/*	
	class SendQuery implements Runnable {
		...
		SendQuery(String c,String r1, String r2,String ke)
		{
			coord = c;
			rep1 = r1;
			rep2 = r2;
        ...
        public void run() {
   		 System.out.println("Checking coordinator alive");
   	     boolean a = alive(coord);
   	     System.out.println("Checked coordinator alive"+a);
   	     		if (a == true) 
   	     		{
   	     			Log.d("Alive function", "Coordinator Alive");
   	     			
   	     			String Q_request = "#!2!"+keyq+"!"+my_address;
   	     			Runnable r1 = new Send_Msg(Q_request,coord);
   	     			new Thread(r1).start();
   	    	 	}
   	     		if (a == false)
   	     		{
   	     			Log.d("Alive function", "Coordinator dead");
   	     			boolean b = alive (rep1);
   	     			if (b == false)
   	     			{
   	     				Log.d("Alive function", "Quorum failed");
   	     			}
   	     			else {
   	     				Log.d("Coordinator dead", "replica alive");
   	     				String Q_request = "#!1!"+keyq+"!"+my_address;
   		     			Runnable r8 = new Send_Msg(Q_request,rep1);
   		     			new Thread(r8).start();
   	     			}
   	     	}
	...
	*/
	...
	protected void ServerThread() {
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				// TODO Auto-generated method stub
				try {
					listenSocket = new ServerSocket(AccessGlobalData.serverPort);
					Log.d("ServerThread", "Server: Server listening on port "
							+ AccessGlobalData.serverPort);
						...
						if (msgType.contentEquals("bootREQ")) {
							if (AccessGlobalData.isDBEmpty) {
								...
								while (!cur.isLast()) {
									_key = cur
											.getString(cur
													.getColumnIndex(KeyValueBase.provider_key));
									_value = cur
											.getString(cur
													.getColumnIndex(KeyValueBase.provider_value));
							...
							cv.put(KeyValueBase.provider_key, recvMsg.key);
							cv.put(KeyValueBase.provider_value, recvMsg.value);
							...
							Uri _uri = Uri.parse("content://"
									+ SimpleDynamoProvider.AUTHORITY1);
							insert(_uri, cv);
							// Risk is here!!
							...
							if (cur.getString(
									cur.getColumnIndex(KeyValueBase.provider_key))
			...
			public void run() {
				...
				try {
					if (sendMsg.messageType.contentEquals("bootREQ")) {
						...
						bootReply = SeparateClientThread.send(sendMsg);
						...
						if (msgType.contentEquals("null")) {
							...
							bootReply = SeparateClientThread.send(bootMsg);
							if (bootReply.messageType.contentEquals("bootACK")) {
								AccessGlobalData.isDataReplicated = true;
								if (bootReply.valueSet.size() > 0) {
									for (int index = 0; index < bootReply.valueSet
											.size(); index++) {
										ContentValues cv = new ContentValues();

										cv.put(KeyValueBase.provider_key,
												bootReply.keySet.get(index));
										cv.put(KeyValueBase.provider_value,
						...
						putReply = SeparateClientThread.send(sendMsg);
						...
						getReply = SeparateClientThread.send(sendMsg);
	...
	/*First Checking the read timeout to see if Coordinator is alive*/
//	 System.out.println("Checking coordinator alive");
//    boolean a = alive(coordinator);
//    System.out.println("Checked coordinator alive"+a);
//    		if (a == true) 
//    		{
//    			Log.d("Alive function", "Coordinator Alive");
//    			/*Now Send the Insert Request to Coordinator*/ 
//    			
//    			String Q_request = "#!2!"+selection+my_address;
//    			Runnable r1 = new Send_Msg(Q_request,coordinator);
//    			new Thread(r1).start();
//   	 	}
//    		if (a == false)
//    		{
//    			Log.d("Alive function", "Coordinator dead");
//    			boolean b = alive (replica1);
//    			if (b == false)
//    			{
//    				Log.d("Alive function", "Quorum failed");
//    			}
//    			else {
//    				Log.d("Coordinator dead", "replica alive");
//    				/*Send Insert Request to replica 1*/
//    				String Q_request = "#!1!"+selection+my_address;
//	     			Runnable r8 = new Send_Msg(Q_request,replica1);
//	     			new Thread(r8).start();
//    			}
//    	}
	/*
	 		Runnable r20 = new SendQuery(coordinator,replica1,replica2,selection);
	 		new Thread(r20).start();
	 
    		while (bloc ==false)
        	{// block query until result comes
        						}
        	bloc = false;
        	// Query result has come now return the cursor for the query result
        // inserting temporarily in local db
        	ContentValues cv = new ContentValues();
			cv.put(myTable.PROVIDER_KEY,Q_key);
			cv.put(myTable.PROVIDER_VALUE,Q_value);
			System.out.println("Inserting result in db ="+Q_value);
			
			//first checking that whether it was present before or not
			SQLiteQueryBuilder qB2 = new SQLiteQueryBuilder();
			qB2.setTables(myTable.DB_TABLE);
			SQLiteDatabase db2 = dab.getWritableDatabase();
			 cursur = qB2.query(db2, null, myTable.PROVIDER_KEY+"="+Q_key, null, null,null, null);
			cursur.setNotificationUri(getContext().getContentResolver(), uri);
			if (cursur.getCount()!=0)
				return cursur;
			
			//if not thn make the cursur
			
			long row = 0;
			SQLiteDatabase sqldbase = dab.getWritableDatabase();
			row = sqldbase.insert(myTable.DB_TABLE, null,cv);
			System.out.println("Inserted");
			// after inserting do the query and get the cursor
			System.out.println("querying");
			
			SQLiteQueryBuilder qB1 = new SQLiteQueryBuilder();
			qB1.setTables(myTable.DB_TABLE);
			SQLiteDatabase db1 = dab.getWritableDatabase();
			 cursur = qB1.query(db1, null, myTable.PROVIDER_KEY+"="+Q_key, null, null,null, null);
			cursur.setNotificationUri(getContext().getContentResolver(), uri);
		System.out.println("Cursor value after farzi add = "+ cursur.getCount());
			//now deleting the additional row we have added
			int z = sqldbase.delete(myTable.DB_TABLE,myTable.PROVIDER_KEY+"="+Q_key, null);
	...
	*/
	...
	protected String replicateData(MessageWrapper msg, String succ, int reqType)
		...
		replicateReply = SeparateClientThread.send(replicateMsg);
	...
	protected String voteForMe(String succ, int reqType) throws IOException {
		...
		voteReply = SeparateClientThread.send(voteMsg);
	...
	protected MessageWrapper askSucc(MessageWrapper msg, int succ, int reqType,
		...
		askSuccReply = SeparateClientThread.send(askSucc);
	...
	static {
		...
		msgProjMap.put(KeyValueBase.MSG_ID, KeyValueBase.MSG_ID);
		msgProjMap.put(KeyValueBase.provider_key, KeyValueBase.provider_key);
		msgProjMap.put(KeyValueBase.provider_value, KeyValueBase.provider_value);
	}
